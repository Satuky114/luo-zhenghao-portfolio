...
def check_remote_switch():
    ...
    try:
        ...
        log(f"📡 远程开关状态: {'✅ 开启' if enabled else '❌ 关闭'}（'{latest[:50]}'）")
        return enabled

    except Exception as e:
        log(f"⚠️ 无法检查开关（{e}），默认: 开启")
...
def main():
    ...
    if not is_clock_time and not (args.manual or args.show):
        log(f"⏰ 当前 {now.strftime('%H:%M')}，不在打卡时段(21:30-23:25)")
